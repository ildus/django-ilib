...
def _build_tree_structure(cls):
    ...
    for obj in cls.get_tree():
        all_nodes[obj.id] = { 'id': int(obj.id), 
                            'children' : map(lambda one: int(one), obj.get_children().values_list('id', flat = True)), 
                            'descendants' : map(lambda one: int(one), obj.get_descendants().values_list('id', flat = True)), 
                            'parent' : int(obj.get_parent().id) if obj.get_parent() else None }
...
class TreeChangeList(main.ChangeList):
    ...
    def get_results(self, request):
        return super(TreeChangeList, self).get_results(request)
...
class TreeEditor(admin.ModelAdmin):
    all_on_page = True  # @todo: pagination
    select_models = [] #показывает дерево только для моделей указанных в списке, выбор осуществляется по полям ниже
    ...
    def indented_short_title(self, item):
        ...
        if hasattr(item, 'get_absolute_url'):
            ...
            class="page_marker" style="width: %dpx;">&nbsp;</span>&nbsp;''' % (
                item.get_absolute_url(), item.id, item.id, 14+item.depth*18)
        else:
            r = '''<span onclick="return page_tree_handler('%s')" id="page_marker-%s" class="page_marker" style="width: %spx;">&nbsp;</span>&nbsp;''' % ( item.id, item.id, 14+item.depth*18)
    ...
    def changelist_view(self, request, extra_context=None, *args, **kwargs):
        ...
        if 'adding_column' not in self.list_display:
            self.list_display.append('adding_column')
    ...
    def _move_node(self, request):
        ...
        if position in ('last-child', 'left'):
            ...
            source = self.model.objects.get(pk=request.POST.get('cut_item'))
            source.save()
    ...
    def adding_column(self, item):
        adds = []
        adds.append(u'<a class="add_next" onclick="return add_next(this);" href="add/?_ref_node_id=%(id)s&_position=right" title="%(desc)s">&nbsp;</a>'% {
            'id': item.pk, "desc": _('add next item') })
        adds.append(u'<a class="add_child" onclick="return add_child(this);" href="add/?_ref_node_id=%(id)s&_position=first-child" title="%(desc)s">&nbsp;</a>'% {
            'id': item.pk, "desc": _('add child item') })
        return ' '.join(adds)
        
    adding_column.allow_tags = True
    adding_column.short_description = _('adding')
